...
export default async function DashboardPage() {
  ...
  return (
    <div className="space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-500 ease-out pb-12">
      {/* Title */}
      <div>
        <h2 className="text-2xl font-bold text-[#D4D4D4] font-display">Overview</h2>
        <p className="text-xs text-[#CCCCCC] mt-1">Comprehensive real-time breakdown of student lead pipelines.</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        <div className="neo-raised p-6 hover:-translate-y-1.5 transition-all duration-300">
          <div className="flex items-center justify-between">
            <h3 className="text-xs font-bold text-[#CCCCCC]">Total Leads</h3>
            <div className="w-8 h-8 rounded-sm bg-gradient-to-br from-[#007ACC] to-[#0062A3] border border-[#3E3E42] flex items-center justify-center text-white">
              <Users className="h-4.5 w-4.5" />
            </div>
          </div>
          <p className="text-3xl font-black text-[#D4D4D4] font-display mt-4">{totalLeads}</p>
        </div>

        <div className="neo-raised p-6 hover:-translate-y-1.5 transition-all duration-300">
          <div className="flex items-center justify-between">
            <h3 className="text-xs font-bold text-[#CCCCCC]">Pipeline Health</h3>
            <div className="w-8 h-8 rounded-sm bg-gradient-to-br from-[#3FC7CE] to-[#0F8A94] border border-[#3E3E42] flex items-center justify-center text-white">
              <Activity className="h-4.5 w-4.5" />
            </div>
          </div>
          <p className="text-3xl font-black text-[#D4D4D4] font-display mt-4">{conversionRate}%</p>
          <p className="text-[10px] text-[#858585] mt-2 font-medium">Ratio of high potential leads (Very Good + Good)</p>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-6">
          {ratingsCards.map(rating => (
            <Link key={rating.name} href={`/dashboard/leads?rating=${encodeURIComponent(rating.name)}`}>
              <div className="neo-raised p-5 hover:border-[#555555] transition-all flex flex-col justify-between h-28">
                <div className="flex items-center justify-between">
                  <span className="text-xs font-bold text-[#CCCCCC]">{rating.name}</span>
                ...
                <p className="text-2xl font-black text-[#D4D4D4] font-display">{rating.count}</p>
              </div>
            </Link>
          ))}
        </div>
            ...
            return (
              <Link key={stage.name} href={`/dashboard/pipeline`} className="block group">
                <div className="flex items-center gap-4">
                  ...
                  <span className="w-10 text-right text-xs font-bold text-[#D4D4D4]">{stage.count}</span>
                </div>
              </Link>
            )
          })}
      ...
      {totalLeads > 0 && (
        <div className="neo-raised p-8">
          <h3 className="text-base font-bold text-[#D4D4D4] mb-6 font-display">Analytics Charts</h3>
