  ...
  return (
    <div className="neo-raised overflow-hidden">
      ...
      {isAdminOrManager && selectedLeadIds.length > 0 && (
        <div className="bg-[#14161A] px-6 py-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-[#111317]/20 animate-in fade-in duration-300">
          <span className="text-xs font-bold text-[#F0F3F8]">{selectedLeadIds.length} leads selected</span>
          <div className="flex items-center gap-3">
            <select 
              value={transferCounselorId} 
              onChange={e => setTransferCounselorId(e.target.value)}
              className="px-3 py-2 bg-[#1B1E23] shadow-[3px_3px_6px_#111317,-3px_-3px_6px_#252A31] text-xs font-bold text-[#AAB4C4] rounded-xl outline-none focus:shadow-[inset_2px_2px_4px_#111317,inset_-2px_-2px_4px_#252A31] transition-all cursor-pointer"
            >
      ...
      <div className="hidden md:block overflow-x-auto">
        <table className="w-full text-left border-collapse min-w-[800px]">
          <thead>
            <tr className="bg-[#14161A] border-b border-[#111317]/20 text-[#AAB4C4] text-[10px] font-bold uppercase tracking-wider">
              {isAdminOrManager && (
          ...
          <tbody className="divide-y divide-[#111317]/20">
            {activeLeads.map((lead) => (
              <tr key={lead.id} className="hover:bg-[#14161A]/20 transition-colors group">
                {isAdminOrManager && (
                ...
                <td className="px-6 py-4">
                  <div className="flex flex-col">
                    <span className="text-xs font-bold text-[#F0F3F8]">{lead.fullName}</span>
                    <span className="text-[10px] text-[#707C91] mt-0.5">{lead.phone ? lead.phone : lead.email}</span>
                  </div>
                </td>
                <td className="px-6 py-4">
                  <StarRating rating={lead.rating} editable={false} size={14} />
                </td>
                <td className="px-6 py-4 text-xs font-semibold text-[#AAB4C4]">
                  {lead.stage}
                </td>
                <td className="px-6 py-4 text-xs font-semibold text-[#AAB4C4]">
                  ...
                  <div className="flex items-center justify-end gap-2.5">
                    {lead.phone && (
                      <a 
                        ...
                        className="inline-flex items-center gap-1.5 px-3 py-2 rounded-xl bg-[#1B1E23] shadow-[3px_3px_6px_#111317,-3px_-3px_6px_#252A31] text-xs font-bold text-[#25D366] hover:shadow-[inset_2px_2px_4px_#111317,inset_-2px_-2px_4px_#252A31] transition-all"
                    ...
                    <Link 
                      ...
                      className="inline-flex items-center gap-1.5 px-3.5 py-2 rounded-xl bg-[#1B1E23] shadow-[3px_3px_6px_#111317,-3px_-3px_6px_#252A31] text-xs font-bold text-[#4855E4] hover:shadow-[inset_2px_2px_4px_#111317,inset_-2px_-2px_4px_#252A31] transition-all whitespace-nowrap"
                    >
            ...
            {activeLeads.length === 0 && (
              <tr>
                <td colSpan={isAdminOrManager ? 7 : 6} className="px-6 py-12 text-center text-xs font-bold text-[#707C91]">
      ...
      <div className="block md:hidden space-y-4 p-2">
        {activeLeads.map((lead, i) => {
          ...
          return (
            <div 
              key={lead.id} 
              className="bg-[#1B1E23] shadow-[3px_3px_6px_#111317,-3px_-3px_6px_#252A31] rounded-2xl p-4 space-y-3.5 border border-[#111317]/10"
            >
              ...
              <div className="flex items-start justify-between gap-3">
                <div className="flex items-center gap-3 min-w-0">
                  ...
                  <div className="min-w-0">
                    <h4 className="text-xs font-bold text-[#F0F3F8] truncate">{lead.fullName}</h4>
                    <p className="text-[10px] text-[#707C91] truncate">
              ...
              <div className="flex items-center justify-between text-xs pt-2 border-t border-[#111317]/10">
                <div className="flex flex-col gap-0.5">
                  <span className="text-[9px] text-[#707C91] font-semibold">Lead Rating</span>
                  <StarRating rating={lead.rating} editable={false} size={11} />
                </div>
                <div className="text-right">
                  <span className="text-[9px] text-[#707C91] block font-semibold">Counselor</span>
                  <span className="text-[10.5px] font-bold text-[#AAB4C4]">
              ...
              <div className="flex items-center gap-3 pt-2">
                {lead.phone && (
                  <a 
                    href={`tel:${lead.phone}`}
                    className="flex-1 h-10 bg-[#1B1E23] shadow-[3px_3px_6px_#111317,-3px_-3px_6px_#252A31] text-emerald-600 rounded-xl hover:shadow-[inset_2px_2px_4px_#111317,inset_-2px_-2px_4px_#252A31] transition-all flex items-center justify-center gap-2 text-xs font-bold"
                  >
                ...
                {lead.phone && (
                  <a 
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="flex-1 h-10 bg-[#1B1E23] shadow-[3px_3px_6px_#111317,-3px_-3px_6px_#252A31] text-[#25D366] rounded-xl hover:shadow-[inset_2px_2px_4px_#111317,inset_-2px_-2px_4px_#252A31] transition-all flex items-center justify-center gap-1.5 text-xs font-bold"
                  >
        ...
        {activeLeads.length === 0 && (
          <div className="neo-raised p-8 text-center text-xs font-bold text-[#707C91]">
